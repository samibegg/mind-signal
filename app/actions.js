...
export async function addPhraseToIdea(ideaId, phrase) {
  ...
  const result = await db.collection("ideas").updateOne(
    { _id: new ObjectId(ideaId), userId: new ObjectId(userId) },
    { $push: { phrases: phrase }, $set: { updatedAt: new Date() } }
  );
  return result.modifiedCount > 0;
}

export async function refineIdeaAction(ideaId, phrases, selectedPromptIds, questionBank, provider) {
  await getUserId(); // Ensure user is authenticated before calling AI
