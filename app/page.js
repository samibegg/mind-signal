        ...
        <div className="space-y-8">
            {Object.entries(refinement).map(([key, value]) => (
                <div key={key}>
                    ...
                    <div className="prose prose-invert max-w-none prose-p:my-2 prose-ul:my-2 prose-li:my-1 prose-strong:text-brand-light-green" 
...
export default function Home() {
  ...
  const handleCreateIdea = (e) => { e.preventDefault(); if (!newIdeaTitle.trim()) return; startTransition(async () => { const newIdea = await addIdea(newIdeaTitle); setIdeas([newIdea, ...ideas]); selectIdea(newIdea, true); setNewIdeaTitle(''); }); };
                  ...
                  <div className="flex flex-col h-full">
                    <div className="p-4 md:p-6 flex-shrink-0 border-b border-dark-border">
                        <div className="flex justify-between items-start pb-4">
                            <div className="flex items-center gap-4"><button onClick={() => setShowDetailView(false)} className="md:hidden p-2 -ml-2 text-dark-text-secondary hover:text-white"><ArrowLeft size={20}/></button><div><h2 className="text-2xl font-bold text-brand-light-green">{activeIdea.title}</h2><p className="text-dark-text-secondary text-xs md:text-sm">STREAM_ID: {activeIdea._id}</p></div></div>
                            <button onClick={handleRefine} disabled={isRefining || activeIdea.phrases.length === 0 || selectedQuestions.size === 0} className="px-4 py-2 bg-brand-purple text-dark-bg font-bold hover:bg-opacity-80 disabled:bg-dark-border disabled:text-dark-text-secondary transition-colors rounded flex items-center text-sm md:text-base">{isRefining ? 'ANALYZING...' : '[ REFINE ]'}</button>
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 pt-4">
                            <div><h3 className="text-dark-text-secondary text-sm mb-2">{'>'} LLM Provider:</h3><div className="flex flex-wrap gap-2">{llmProviders.map(p => (<button key={p.id} onClick={() => setLlmProvider(p.id)} className={`px-2 py-1 text-xs rounded transition-colors ${llmProvider === p.id ? 'bg-brand-purple text-dark-bg' : 'bg-dark-surface hover:bg-dark-border text-dark-text-secondary'}`}>{`[ ${p.label} ]`}</button>))}</div></div>
                            <div><h3 className="text-dark-text-secondary text-sm mb-2">{'>'} Analysis Modules:</h3><div className="flex flex-wrap gap-2">{questionBank.map(q => (<button key={q.id} onClick={() => handleQuestionToggle(q.id)} className={`px-2 py-1 text-xs rounded transition-colors ${selectedQuestions.has(q.id) ? 'bg-brand-green text-dark-bg' : 'bg-dark-surface hover:bg-dark-border text-dark-text-secondary'}`}>{selectedQuestions.has(q.id) ? `[ ${q.label} ✓]` : `[ ${q.label} ]`}</button>))}</div></div>
                        </div>
                    </div>
